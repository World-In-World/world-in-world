# modified from amsolver, parallelized data collection script
...
# use slide_block_to_color_target as an example
# slide_block_to_color_target has 4 variations
flags.DEFINE_string('save_path',
                    './data',
                    'Where to save the demos.')
flags.DEFINE_list('tasks', ["slide_block_to_color_target"],
                  'The tasks to collect. If empty, all tasks are collected.')
flags.DEFINE_integer('episodes_per_variation', 25,
...
def run(i, lock, task_index, variation_count, results, file_lock, tasks):
    ...
    all the episodes_per_variation for that variation."""
    ...
    while True:
        ...
        with lock:
            ...
            if task_index.value >= num_tasks:
                print('Process', i, 'finished', flush=True)
                break
        ...
        while ex_idx < FLAGS.episodes_per_variation:
            print('Process', i, '// Task:', task_env.get_name(),
                  '// Variation:', my_variation_count, '// Demo:', ex_idx, flush=True)
            attempts = 10
            while attempts > 0:
                try:
                    task_env = amsolver_env.get_task(t)
...
def main(argv):
    ...
    if FLAGS.all_variations:
        raise NotImplementedError('multiprocessing for all_variations not supported for now')
    else:
        processes = [Process(
            target=run, args=(
    ...
    print('Data collection done!', flush=True)
    for i in range(FLAGS.processes):
        print(result_dict[i], flush=True)
