...
class VLMBenchEnv(gym.Env):
    ...
    def __init__(self, task_class, observation_mode='state',
                 render_mode: Union[None, str] = None, n_obs_steps=1):
        ...
        elif observation_mode == 'front_horizon': # for dp3
            obs_config.set_all_low_dim(True)
            obs_config.set_front_camera(True)
        else:
            raise ValueError(
        ...
        if observation_mode == 'front_horizon':
            self.obs_seq = deque([obs], maxlen=self.n_obs_steps+1)
        ...
        elif observation_mode == 'front_horizon':
            self.observation_space = spaces.Dict({
                "state": spaces.Box(
                    low=-np.inf, high=np.inf,
                    shape=obs.get_low_dim_data().shape),
