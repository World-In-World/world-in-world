#!/bin/bash
# Usage: bash downstream/scripts/init_managers.sh "4,5,6,7" num_workers_sam2 exp_id
# where "4,5,6,7" is the CUDA_VISIBLE_DEVICES value,
# num_workers_sam2 is the number of workers for the 'sam2' command,
# and exp_id is the experiment ID.

# Check that exactly 3 arguments are provided.
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 num_workers_sam2 exp_id"
    exit 1
fi
