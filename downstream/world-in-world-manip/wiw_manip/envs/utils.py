...
RLBENCH_SCENE_BOUNDS = np.array([-0.3, -0.5, 0.6, 0.7, 0.5, 1.6])
...
# skipping loading YOLO for libero backend
...
def extract_obs(obs: Observation, target_obj=None, objects_name=None, use_wrist=True) -> Dict[str, np.ndarray]:
    ...
    cond = cond & (RLBENCH_SCENE_BOUNDS[0] < pc_flat[..., 0]) & (pc_flat[..., 0] < RLBENCH_SCENE_BOUNDS[3]) \
                & (RLBENCH_SCENE_BOUNDS[1] < pc_flat[..., 1]) & (pc_flat[..., 1] < RLBENCH_SCENE_BOUNDS[4]) \
                & (RLBENCH_SCENE_BOUNDS[2] < pc_flat[..., 2]) & (pc_flat[..., 2] < RLBENCH_SCENE_BOUNDS[5])
    idx = np.where(cond)[0]
    if len(idx) >= 2048:
...
def point_to_voxel_index(point: np.ndarray, bounds=None):
    bounds = RLBENCH_SCENE_BOUNDS if bounds is None else np.asarray(bounds, dtype=np.float64)
...
def describe_discrete_action_conversion(
    ...
    is_gripper_open = round(discrete_action[-1])
    ...
    # )
    return np.asarray(continuous_action)
...
def get_continous_action_from_discrete(discrete_action, bounds=None, flip_y=False):
    ...
    """
    continuous_action = describe_discrete_action_conversion(
        discrete_action,
        bounds=bounds,
        flip_y=flip_y,
        zero_rotation_as_identity_wxyz=False,
...
def draw_xyz_coordinate(image_path, resolution, backend="rlbench"):
    ...
    if backend == "libero":
        ...
        if resolution in origins.keys():
            origin = origins[resolution]  # Adjust based on the table's position in the image

            # Define axis lengths
            axis_length = axis_lengths[resolution]  # Length of each axis
            ...
            cv2.putText(
                    ...
                    text_positions[resolution],        # Position for the text
            ...
            cv2.arrowedLine(image, origin, (origin[0] - axis_length + x_axis_offsets[resolution], origin[1] + axis_length), color_x, 2, tipLength=0.2)
...
def get_default_scene_bounds(backend="rlbench"):
    if backend == "libero":
        ...
        return RLBENCH_SCENE_BOUNDS.copy()
    else:
...
def form_object_coord_for_input(obs, task_class, camera_types, backend, env=None):
    if backend == "libero":
        if env is None:
