...
class TaskEnvironment(object):
    ...
    def _path_action(self, action, collision_checking=False, relative_to=None, recorder=None):
        ...
        if not done:
            path = self._path_action_get_path(
                action, collision_checking, relative_to)    #_joint_position_action
            small_step = 0
            while not done:
                done = path.step()
                self._scene.step()
    ...
    def step(self, action, collision_checking=None, use_auto_move=True, recorder=None, need_grasp_obj=None) -> Tuple[Observation, int, bool]:
        ...
        open_condition = all(x > 0.9 for x in self._robot.gripper.get_open_amount())    #whether gripper is fully open
        open_condition &= (len(self._robot.gripper.get_grasped_objects())==0)   #not holding any objects
        ...
        if current_ee != ee_action:
            ...
            while not done:
                ...
                if self._enable_path_observations:
                    obs = self._scene.get_observation()
                    self._path_observations.append(obs)
            if ee_action == 1.0:
    ...
    def load_config(self, task_base, waypoint_sets, config_path):
        ...
        self._reset_called=True
        print(config.high_level_descriptions)
