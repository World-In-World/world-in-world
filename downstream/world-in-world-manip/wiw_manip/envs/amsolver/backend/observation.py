...
class Observation(object):
    ...
    def get_low_dim_data(self) -> np.ndarray:
        ...
        return np.concatenate(low_dim_data).astype(np.float32) if len(low_dim_data) > 0 else np.array([], dtype=np.float32)
    
    def get_euler_gripper_pose(self) -> np.ndarray:
        quat = self.gripper_pose[3:7]  # [T, 4]
        euler = R.from_quat(quat).as_euler('xyz', degrees=False)  # [T, 3]
        action = np.concatenate([self.gripper_pose[:3], euler, [self.gripper_open]], axis=-1) # [T, 7]
        return action
