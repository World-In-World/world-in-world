...
from amsolver.backend.conditions import JointCondition, ConditionSet
...
color_permutations = [color_permutations[(7 * i) % len(color_permutations)] for i in range(len(color_permutations))]
...
class PushButtons(Task):
    ...
    def init_episode(self, index: int) -> List[str]:
        ...
        # For each color permutation, we want to have 1 button pushed
    ...
    def variation_count(self) -> int:
        assert len(color_permutations) * MAX_TARGET_BUTTONS == MAX_VARIATIONS
        return np.minimum(
