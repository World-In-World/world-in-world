...
from wiw_manip.planner.planner_config.generation_guide import llm_generation_guide, vlm_generation_guide
from wiw_manip.planner.planner_config.generation_guide_manip import llm_generation_guide_manip, vlm_generation_guide_manip
from wiw_manip.planner.utils.planner_utils import convert_format_2claude, convert_format_2gemini, ActionPlan_1, ActionPlan, ActionPlan_lang, \
...
class RemoteModel:
    def __init__(
        ...
        self.extra_body = {"top_k": top_k, "top_p": 1.0, "temperature": temperature}       # tell vLLM “no hard cap”

        if self.model_type == 'local':
            os.environ["CUDA_VISIBLE_DEVICES"] = "4,5,6,7"
            backend_config = PytorchEngineConfig(session_len=12000, dtype='float16', tp=tp, block_size=1)
            ...
            if "claude" in self.model_name:
                ...
                self.model = OpenAI(base_url = remote_url, api_key="xxxx", timeout=TIMEOUT_SECS)
    ...
    def respond(self, message_history: list, **kwargs):
        extra_body = deepcopy(self.extra_body)
        for k in extra_body.keys() & kwargs.keys():  # set intersection
            if kwargs[k] is not None:
                extra_body[k] = kwargs[k]
            kwargs.pop(k)  # remove from kwargs to avoid duplication

        kwargs.update({"extra_body": extra_body})
        if self.model_type == 'local':
            ret = self._call_local(message_history, **kwargs)
        else:
            if "claude" in self.model_name:
                ret = self._call_claude(message_history, **kwargs)
            elif "gemini" in self.model_name:
                ret = self._call_gemini(message_history, **kwargs)
            elif "gpt" in self.model_name:
                ret = self._call_gpt(message_history, **kwargs)
            elif 'qwen' in self.model_name:
                ret = self._call_gpt(message_history, **kwargs)
            elif "Qwen2-VL-7B-Instruct" in self.model_name:
                ret = self._call_qwen7b(message_history, **kwargs)
            elif "Qwen2.5-VL-7B-Instruct" in self.model_name:
                ret = self._call_qwen7b(message_history, **kwargs)
            elif "Qwen2-VL-72B-Instruct" in self.model_name:
                ret = self._call_qwen72b(message_history, **kwargs)
            elif "Qwen2.5-VL-72B-Instruct" in self.model_name:
                ret = self._call_qwen72b(message_history, **kwargs)
            elif "Llama-3.2-11B-Vision-Instruct" in self.model_name:
                ret = self._call_llama11b(message_history, **kwargs)
            elif "meta-llama/Llama-3.2-90B-Vision-Instruct" in self.model_name:
                ret = self._call_qwen72b(message_history, **kwargs)
            elif "90b-vision-instruct" in self.model_name:
                ret = self._call_llama90(message_history, **kwargs)
            elif "OpenGVLab/InternVL" in self.model_name:
                ret = self._call_intern38b(message_history, **kwargs)
            # elif "OpenGVLab/InternVL2_5-38B" in self.model_name:
            #     ret = self._call_intern38b(message_history)
    ...
    def _call_local(self, message_history: list, **kwargs):
    ...
    def _call_claude(self, message_history: list, **kwargs):
    ...
    def _call_gemini(self, message_history: list, **kwargs):
    ...
    def _call_gpt(self, message_history: list, **kwargs):
    ...
    def _call_qwen7b(self, message_history: list, **kwargs):
    ...
    def _call_llama90(self, message_history: list, **kwargs):
        if self.task_type == "manip":
            response = self.model.chat.completions.create(
    ...
    def _call_llama11b(self, message_history, **kwargs):
    ...
    def _call_qwen72b(self, message_history, **kwargs):
        if not self.language_only:
            message_history = convert_format_2gemini(message_history)
        ...
        outs = [response.choices[i].message.content for i in range(len(response.choices))]
        return outs

    def _call_intern38b(self, message_history, **kwargs):
