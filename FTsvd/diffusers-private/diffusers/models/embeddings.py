...
def get_timestep_embedding(
    ...
    exponent = -math.log(max_period) * torch.arange(    #torch.Size([128])
        start=0, end=half_dim, dtype=torch.float32, device=timesteps.device
    )
    exponent = exponent / (half_dim - downscale_freq_shift)

    emb = torch.exp(exponent)   #all positive in (0,1)
    emb = timesteps[:, None].float() * emb[None, :] #torch.Size([3, 128])

    # scale embeddings
    emb = scale * emb

    # concat sine and cosine embeddings
    emb = torch.cat([torch.sin(emb), torch.cos(emb)], dim=-1)   #torch.Size([3, 256])
...
class Timesteps(nn.Module):
    def __init__(self, num_channels: int, flip_sin_to_cos: bool, downscale_freq_shift: float, scale: int = 1):
        super().__init__()
        self.num_channels = num_channels    #320 or 256
        self.flip_sin_to_cos = flip_sin_to_cos      #True
        self.downscale_freq_shift = downscale_freq_shift    #default is 0
        self.scale = scale      #1
...
class ActionEmbedder(nn.Module):
    def __init__(
        ...
        """
        Args:
            in_dim (int): Dimension of the input (e.g. one-hot action vector size).
            out_dim (int): Dimension of the output embedding.
            max_seq_len (int): Maximum sequence length for the positional embeddings.
            act_fn (str): Activation function to use (default "silu").
            sample_proj_bias (bool): Whether to use bias in the linear layers.
        """
        super().__init__()
        # Build an MLP using nn.Sequential.
        self.layers = nn.Sequential(
            nn.Linear(in_dim, 256, bias=sample_proj_bias),
            get_activation(act_fn),
            nn.Linear(256, 512, bias=sample_proj_bias),
            get_activation(act_fn),
            nn.Linear(512, out_dim, bias=sample_proj_bias)
        )

        # Create a learnable positional embedding of shape (max_seq_len, out_dim)
        self.pos_embedding = nn.Parameter(torch.randn(max_seq_len, out_dim))
        self.out_dim = out_dim

    def forward(self, sample, condition=None):
        """
        Args:
            sample (torch.Tensor): Tensor of shape (batch_size, seq_len, in_dim)

        Returns:
            torch.Tensor: Tensor of shape (batch_size, seq_len, out_dim) with positional embeddings added.
        """
        ...
        for idx in range(bs):
            ...
            if (curr_sample == -1).all():
                curr_sample = torch.zeros(
                    curr_sample.shape[0], self.out_dim,
                    dtype=dtype, device=sample.device,
                )
            else:
                # Pass the sample through the MLP.
                curr_sample = self.layers(curr_sample)

                # Get the current sequence length.
                seq_len = curr_sample.shape[0]
                assert seq_len == self.pos_embedding.shape[0]
                # pos_emb shape: (seq_len, out_dim)
                # Add positional embeddings (broadcasting to the batch dimension).
                curr_sample = curr_sample + self.pos_embedding
...
class ActionEmbedder_(nn.Module):
    def __init__(self, num_frames, embed_dim, input_channel, zero_init=False):
        super().__init__()
        # self.input_channel = 4
        self.input_channel = input_channel             # for manipulation tasks "23", for nav "14"
        self.proj = nn.Linear(self.input_channel*12, embed_dim)    #25 is the max length of action seq
    ...
    def forward(self, x):
        # Expect x of shape (bs, num_frames, num_frames)
        bs, n, m = x.shape
        assert n == self.num_frames and m == self.input_channel, (
            f"Expected input shape (bs, {self.num_frames}, {self.num_frames}), got {x.shape}"
        )

        # For each element in x, compute:
        features = torch.stack([
            ...
            torch.cos(x * 10.0),
            torch.sin(x * 10.0),
        ], dim=-1)  # Shape: (bs, num_frames, num_frames, 6)

        # Flatten the last two dimensions so that each row becomes a feature vector
        # of length num_frames * 12
        features = features.view(bs, self.num_frames, self.input_channel*12) #(1,14,12*14)

        # Convert to the projection's dtype and apply the linear layer
        dtype = self.proj.weight.dtype
        h = self.proj(features.to(dtype=dtype))
        return h
