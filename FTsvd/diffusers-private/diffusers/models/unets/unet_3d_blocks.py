...
from ..transformers.transformer_temporal import (
    ...
    TransformerSpatioTemporalModel_A,
)
...
class UNetMidBlockSpatioTemporal(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            if action_attention_dim is not None:
                atten_moudle = TransformerSpatioTemporalModel_A(
                    num_attention_heads,
                    in_channels // num_attention_heads,
                    in_channels=in_channels,
                ...
                self.action_attention_dim = action_attention_dim
            else:
                atten_moudle = TransformerSpatioTemporalModel(
                    num_attention_heads,
                    in_channels // num_attention_heads,
                    in_channels=in_channels,
                    num_layers=transformer_layers_per_block[i],
            ...
            attentions.append(atten_moudle)
        ...
        **kwargs,
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if self.training and self.gradient_checkpointing:  # TODO
                ...
                if hasattr(self, 'action_attention_dim'):
                    hidden_states = attn(
                        hidden_states,
                        encoder_hidden_states=encoder_hidden_states,
                        image_only_indicator=image_only_indicator,
                        return_dict=False,
                        action_embeds=kwargs['action_embeds'],
                    )[0]
                else:
                    hidden_states = attn(
                        hidden_states,
            ...
            if hasattr(self, 'action_attention_dim'):
                attentions.append(
                    TransformerSpatioTemporalModel_A(
                        ...
                        num_layers=transformer_layers_per_block[i],
                        cross_attention_dim=cross_attention_dim,
                        action_attention_dim=self.action_attention_dim,
                    )
                )
            else:
                attentions.append(
