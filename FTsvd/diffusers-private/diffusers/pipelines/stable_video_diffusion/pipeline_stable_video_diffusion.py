...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def _encode_image(
        ...
        if not isinstance(image, torch.Tensor):
            ...
            image = self.video_processor.numpy_to_pt(image)  # range [0, 1]

            # We normalize the image before resizing to match with the original implementation.
            # Then we unnormalize it after resizing.
            image = image * 2.0 - 1.0
            image = _resize_with_antialiasing(image, (224, 224))
            image = (image + 1.0) / 2.0     # range [0, 1]
    ...
    def prepare_latents(
        ...
        added_action_ids: Optional[torch.Tensor] = None,
    ):
        shape = (
            batch_size,
            num_frames,
        ...
        if latents is None:
            if added_action_ids is not None and self.unet.task_type == "navigation":
                actions = decode_action_seq_frames(added_action_ids)
                latents = sample_latent_noise(actions, shape, device, dtype, generator)
            else:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if kwargs.get("past_obs_imgs") is not None:
            past_obs_imgs = kwargs["past_obs_imgs"]
            img_embeds = []
            for img in past_obs_imgs:
                img_embed = self._encode_image(img, device, num_videos_per_prompt, self.do_classifier_free_guidance)   #torch.Size([2, 1, 1024]), image_latents[0] is all zeros if do_classifier_free_guidance=True
                img_embeds.append(img_embed)
            image_embeddings = torch.cat(img_embeds, dim=1)  #torch.Size([2, 5, 1024])
        else:
        ...
        image_latents = self._encode_vae_image( #torch.Size([2, 4, 32, 64]), image_latents[0] is all zeros
            image,
            device=device,
        ...
        image_latents = image_latents.unsqueeze(1).repeat(1, num_frames, 1, 1, 1)   #torch.Size([2, 25, 4, 32, 64])
        ...
        # 7. Prepare latent variables     #generative rand noise
        num_channels_latents = self.unet.config.in_channels
        latents = self.prepare_latents(   #torch.Size([1, 14, 4, 32, 64])
            ...
            generator,
            latents,
            kwargs.get("added_action_ids", None),
        )
        ...
        guidance_scale = _append_dims(guidance_scale, latents.ndim)     #torch.Size([1, 25, 1, 1, 1])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    ...
                    return_dict=False,
                    added_action_ids=kwargs.get("added_action_ids", None),
                )[0]
...
def rotate_by_degrees(input_z: torch.Tensor, angle: float) -> torch.Tensor:
    ...
    if angle not in ANGLE_TO_DENOM:
...
def decode_action_seq_frames(action_seq_frames: torch.Tensor) -> torch.Tensor:
    """
    Reverts the action_ids_idx_encode operation.
    Args:
        action_seq_frames (Float[Tensor, 'B L L']): Tensor of shape
            (batch_size, sequence_length, action_max_len) where action_max_len
            is assumed to be equal to sequence_length. Each row in a sample's
            matrix contains a cumulative encoding, and the i-th element of the
            i-th frame holds the intended action id.
    Returns:
        torch.Tensor: A tensor of shape (batch_size, sequence_length) containing
            the decoded action ids.
    """
    # Extract the diagonal from each sample's matrix.
    # This returns a tensor of shape (B, L) where each element (b, i) corresponds
    # to action_seq_frames[b, i, i]
    decoded_actions = action_seq_frames.diagonal(dim1=-2, dim2=-1)
